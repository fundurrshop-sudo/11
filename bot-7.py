...
DEFAULT_SETTINGS = {
    ...
    "success": (
        "✅ Заказ оформлен!\n\n"
        "💳 Переведите {price} по СБП на номер {phone} и обязательно укажите в комментарии "
        "к переводу код:\n\n"
        "「 {code} 」\n\n"
    ...
    "sbp_phone": "не указан — задайте через /set_sbp_phone",
}
...
EDITABLE_TEXTS = {
    ...
    "success": "финальное сообщение с реквизитами (доступны {price}, {phone}, {code})",
}
...
EDITABLE_LABELS = {
    ...
    "sbp_phone": "номер телефона для перевода по СБП",
}
...
async def order_confirm(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    phone = await get_setting_text("sbp_phone")
    text, photo = await get_setting("success")
    final_text = text.replace("{price}", data["price"]).replace("{code}", payment_code).replace("{phone}", phone)
    if photo:
    ...
    if ADMIN_CHAT_ID:
        ...
        notify_text += f"Код для СБП: {payment_code} (проверить и подтвердить: /confirm {payment_code})"
        try:
...
async def cmd_admin(message: Message):
    ...
    text += "\n\nПодписи кнопок, полей и номер СБП (только текст):\n"
