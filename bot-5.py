...
# Куда приходят заказы и кто имеет доступ к админ-командам
ADMIN_CHAT_ID = 5245002499
ADMIN_IDS = [5245002499]

# Путь к базе данных (на Railway монтируется Volume в /data, чтобы данные не терялись)
...
DEFAULT_SETTINGS = {
    ...
    "success": "✅ Заказ оформлен!\n\n⏳ Ожидайте, сейчас с вами свяжется поставщик.",
    "success_sbp": (
        "✅ Заказ оформлен!\n\n"
        "💳 Переведите {price} по СБП и обязательно укажите в комментарии к переводу код:\n\n"
        "「 {code} 」\n\n"
        "⏳ После оплаты дождитесь подтверждения от продавца."
    ),
    "btn_pay_sbp": "💳 По СБП",
    "btn_pay_card": "💳 По номеру карты",
...
EDITABLE_TEXTS = {
    ...
    "payment_prompt": "текст выбора способа оплаты",
    "confirm_header": "заголовок экрана проверки заказа",
    "success": "финальное сообщение после оформления заказа (для оплаты картой и др.)",
    "success_sbp": "финальное сообщение при оплате по СБП (доступны {price} и {code})",
}

EDITABLE_LABELS = {
    "btn_pay_sbp": "кнопка оплаты СБП",
    "btn_pay_card": "кнопка оплаты картой",
...
async def init_db():
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        # Миграция: если таблица orders была создана раньше без колонки payment_code — добавляем.
        try:
...
async def ask_payment_method(message: Message, state: FSMContext):
    btn_sbp = await get_setting_text("btn_pay_sbp")
    btn_card = await get_setting_text("btn_pay_card")
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=btn_sbp, callback_data="pay:sbp")],
        [InlineKeyboardButton(text=btn_card, callback_data="pay:card")],
    ])
    await send_styled(message, "payment_prompt", reply_markup=kb)
    await state.set_state(OrderStates.waiting_payment)


@user_router.callback_query(OrderStates.waiting_payment, F.data.startswith("pay:"))
async def receive_payment_method(callback: CallbackQuery, state: FSMContext):
    method_code = callback.data.split(":")[1]
    label = await get_setting_text("btn_pay_sbp" if method_code == "sbp" else "btn_pay_card")
    await state.update_data(payment_method=label, payment_method_code=method_code)
...
async def show_confirmation(message: Message, state: FSMContext):
    ...
    details += f"{label_payment}: {data['payment_method']}"
...
async def order_confirm(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    is_sbp = data.get("payment_method_code") == "sbp"
    payment_code = await generate_unique_code() if is_sbp else None
    ...
    if is_sbp:
        text, photo = await get_setting("success_sbp")
        final_text = text.replace("{price}", data["price"]).replace("{code}", payment_code)
        if photo:
            await callback.message.answer_photo(photo, caption=final_text)
        else:
            await callback.message.answer(final_text)
    else:
        await send_styled(callback.message, "success")
    ...
    if ADMIN_CHAT_ID:
        ...
        notify_text += f"Способ оплаты: {data['payment_method']}\n"
        if payment_code:
            notify_text += f"Код для СБП: {payment_code} (ждём перевод с этим кодом в комментарии)"
        try:
...
async def cmd_admin(message: Message):
    ...
    text = (
        ...
        "/confirm <код> — подтвердить оплату по СБП: показывает покупателя, товар и почту, "
        "и отмечает заказ как оплаченный\n\n"
        "Админы:\n"
        "/addadmin <user_id> — назначить админа\n"
        "/deladmin <user_id> — снять админа\n\n"
        "Тексты сообщений (пришлите текст или фото с подписью, чтобы задать и текст, и фото сразу):\n"
    )
    text += "\n".join(f"/set_{key} — {label}" for key, label in EDITABLE_TEXTS.items())
    text += "\n\nПодписи кнопок и полей (только текст):\n"
...
def register_edit_handler(key: str, supports_photo: bool):
    async def start_edit(message: Message, state: FSMContext):
        ...
        if supports_photo:
            ...
            await message.answer("Пришлите новый текст (это подпись кнопки/поля, без фото):")
...
async def edit_receive_photo(message: Message, state: FSMContext):
    ...
    await set_setting_photo(key, photo_id, value=caption)
...
async def edit_receive_text(message: Message, state: FSMContext):
    ...
    if text.lower() == "убрать фото" and data.get("edit_supports_photo"):
        ...
        await message.answer("✅ Фото убрано.")
    else:
        await set_setting_text(key, text)
        await message.answer("✅ Обновлено.")
    await state.clear()
...
async def addproduct_photo(message: Message, state: FSMContext):
    photo_id = message.photo[-1].file_id
    await finish_add_product(message, state, photo_id)
...
async def finish_add_product(message: Message, state: FSMContext, photo_id):
    data = await state.get_data()
    category_id = await get_or_create_category(data["category_name"])
    product_id = await add_product(
        category_id, data["product_name"], data["price"], data["requires_email"], photo_id
    )
    summary = (
...
async def setproductphoto_receive(message: Message, state: FSMContext):
    data = await state.get_data()
    photo_id = message.photo[-1].file_id
    await set_product_photo(data["product_id"], photo_id)
...
async def setproductphoto_clear(message: Message, state: FSMContext):
    data = await state.get_data()
    await set_product_photo(data["product_id"], None)
    await message.answer("✅ Фото товара убрано.")
    await state.clear()
