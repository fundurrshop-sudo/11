...
EDITABLE_TEXTS = {
    ...
    "success": "финальное сообщение после оформления заказа",
}
...
async def create_order(user_id: int, username: str, product_id: int, product_name: str,
                        price: str, email: str, payment_method: str) -> int:
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute(
            """INSERT INTO orders (user_id, username, product_id, product_name, price, email, payment_method)
               VALUES (?, ?, ?, ?, ?, ?, ?)""",
            (user_id, username, product_id, product_name, price, email, payment_method),
        )
        await db.commit()
        return cur.lastrowid
...
async def list_recent_orders(limit: int = 20):
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute(
            "SELECT id, username, user_id, product_name, price, status, created_at FROM orders ORDER BY id DESC LIMIT ?",
            (limit,),
        )
        return await cur.fetchall()
...
async def receive_payment_method(callback: CallbackQuery, state: FSMContext):
    ...
    label = await get_setting_text("btn_pay_sbp" if code == "sbp" else "btn_pay_card")
    await state.update_data(payment_method=label)
...
async def cmd_orders(message: Message):
    ...
    if not orders:
        ...
        return
    lines = []
    for oid, username, uid, pname, price, status, created in orders:
        who = f"@{username}" if username else str(uid)
        lines.append(f"#{oid} | {who} | {pname} ({price}) | {status} | {created}")
